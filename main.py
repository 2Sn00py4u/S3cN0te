...
class API:
    ...
    def __init__(self, dbms: duck.DBMS, host: str = "localhost", port: int = 55555, session_type: str = "filesystem", session_file_dir: str|plib.Path = plib.Path("sessions"), session_file_threshold: int = 500,
                 session_permanent: bool = True, session_use_signer: bool = True, permanent_session_lifetime: datetime.timedelta = datetime.timedelta(minutes=10), debug: bool = False) -> "API":
                ...
                match request.method:
                    ...
                    case "POST":
                        try:
                            ...
                            req = data["req"]
                            if req == "edit?":
                                return jsonify({"status": "success", "edit": "new", "pages": [{"id": "0", "markdown": "# SecNote\n"}]}), 200
...
if __name__ == "__main__":
    ...
    secnoteAPI.configSession("secnoteSessionKey")   # <-- Kann nach belieben geändert werden
    secnoteAPI.initAPI()
    secnoteAPI.run()